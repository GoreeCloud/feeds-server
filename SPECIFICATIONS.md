...
Database engine, durable deduplication index/storage model, job scheduler, search backend, cache technology, authenticated parser/API exposure, remote retrieval strategy, HTML sanitization policy, authentication/session implementation, packaging format, container image, production port/hostname, and production topology remain unresolved.
