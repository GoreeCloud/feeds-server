...
Schema/migration design for article categories/tags/media/images and complete subscription settings, retention execution, server startup/configuration wiring for PostgreSQL, production TLS/credential injection, retrieval scheduler/queue/adaptive refresh/retry/backoff/history design, search backend, cache technology, authenticated ingestion/API exposure, HTML sanitization policy, authentication/session implementation, packaging format, container image, production port/hostname, and production topology remain unresolved.
